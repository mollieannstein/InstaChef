...
rId_array = [17795, 9307, 17551, 1203]

rId_array.each do |rId|
  uri = URI.parse(URI.escape("http://food2fork.com/api/get?key=API#{API_KEY}&rId=#{rId}"))
  ...
  directions.each do |step|
    Direction.create(step: step, recipe_id: r.id)
  end
  ...
  r.servings = doc.css('div.subtext').first.text
  r.save!
end
